...
| PostgreSQL session/activation/payment store | Implemented and tested | A separate bounded `--migrate` process owns DDL; normal app startup uses table DML only. Existing deployments must transfer ownership before adopting the split. |
...
[DEPLOYMENT.md](DEPLOYMENT.md), a reviewed four-repository commit manifest, and
synchronized backend, portal, cardholder, and POS tags created from the public
histories. Passing CI alone is source evidence, not deployment certification.
